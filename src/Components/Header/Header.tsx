...
function Header() {
  ...
  async function addTodo(name, description) {
    const { data, error } = await supabase.from('todos').insert([{ title: title, description: description }]).select('*')
    if(error){
      console.log('This insertion is not done')
    }
    else{
      console.log("add succefully"+ data)
    }
              ...
              >
                <div className="relative p-4 w-full max-w-md max-h-full">
                  <div className="relative bg-white rounded-lg shadow-sm">
                    ...
                    <form onSubmit={handleSumbit} className="p-4 md:p-5">
                      <div className="mb-4">
                        <input
                          type="text"
                          name="title"
